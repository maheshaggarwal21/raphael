...
[![tests](https://img.shields.io/badge/tests-774%20passing-brightgreen)](test/)
...
| **774 tests, 6 CI combos** | plain `node:test`, no frameworks, run on Linux + Windows × Node 18/20/22 — plus a zero-token *canary gate* that re-proves the security chokepoint on every push. |
...
```
 ...
 proof     test/ (774) · src/eval/ (6 scenarios + 6 canaries) · CI (Linux+Windows × Node 18/20/22)
```
...
```bash
npm install
npm test                  # 774 tests, plain node:test, no frameworks
