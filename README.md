...
[![tests](https://img.shields.io/badge/tests-757%20passing-brightgreen)](test/)
...
| **757 tests, 6 CI combos** | plain `node:test`, no frameworks, run on Linux + Windows × Node 18/20/22 — plus a zero-token *canary gate* that re-proves the security chokepoint on every push. |
...
```
 ...
 engine    src/lib/*.js         64 modules, plain ESM, 2 runtime deps
 ...
 proof     test/ (757) · src/eval/ (6 scenarios + 6 canaries) · CI (Linux+Windows × Node 18/20/22)
```
...
```bash
npm install
npm test                  # 757 tests, plain node:test, no frameworks
