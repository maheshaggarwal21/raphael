...
[![tests](https://img.shields.io/badge/tests-592%20passing-brightgreen)](test/)
...
| **592 tests, 6 CI combos** | plain `node:test`, no frameworks, run on Linux + Windows × Node 18/20/22 — plus a zero-token *canary gate* that re-proves the security chokepoint on every push. |
...
```
 ...
 engine    src/lib/*.js         59 modules, plain ESM, 2 runtime deps
 ...
 proof     test/ (592) · src/eval/ (6 scenarios + 6 canaries) · CI (Linux+Windows × Node 18/20/22)
```
...
```bash
npm install
npm test                  # 592 tests, plain node:test, no frameworks
