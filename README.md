...
[![tests](https://img.shields.io/badge/tests-685%20passing-brightgreen)](test/)
...
| **685 tests, 6 CI combos** | plain `node:test`, no frameworks, run on Linux + Windows × Node 18/20/22 — plus a zero-token *canary gate* that re-proves the security chokepoint on every push. |
...
# 📖 The command atlas — all 43 verbs
...
```
 ...
 engine    src/lib/*.js         61 modules, plain ESM, 2 runtime deps
 ...
 proof     test/ (685) · src/eval/ (6 scenarios + 6 canaries) · CI (Linux+Windows × Node 18/20/22)
```
...
```bash
npm install
npm test                  # 685 tests, plain node:test, no frameworks
