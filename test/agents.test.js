...
// 21.13 — the shipped plugin files are GENERATED by scripts/build-agents.mjs,
// which is a manual step: `npm test` never imported them and CI never re-rendered
// them, so editing a mission and forgetting to rebuild shipped a plugin whose
// agents disagreed with the source of truth — and every test still passed
// (audit 2026-07-26). They were in sync by discipline, not by construction.
test('every shipped plugin agent file is byte-identical to renderAgent(agents.js)', () => {
  const dir = path.join(ROOT, 'plugin', 'agents');
  assert.ok(existsSync(dir), 'plugin/agents must exist');

  for (const a of AGENTS) {
    const file = path.join(dir, `raphael-${a.slug}.md`);
    assert.ok(existsSync(file), `missing generated file for "${a.slug}" — run: node scripts/build-agents.mjs`);
    assert.equal(
      readFileSync(file, 'utf8'),
      renderAgent(a),
      `plugin/agents/raphael-${a.slug}.md is STALE — run: node scripts/build-agents.mjs`
    );
  }

  // and no orphans left behind by a removed or renamed agent
  const expected = new Set(AGENTS.map((a) => `raphael-${a.slug}.md`));
  const orphans = readdirSync(dir).filter((f) => f.endsWith('.md') && f !== 'README.md' && !expected.has(f));
  assert.deepEqual(orphans, [], 'orphaned generated agent file(s) — delete them or restore the roster entry');
});

test('every shipped recipe file is byte-identical to renderRecipe(agents.js)', () => {
  const dir = path.join(ROOT, 'plugin', 'recipes');
  assert.ok(existsSync(dir), 'plugin/recipes must exist');
  for (const r of RECIPES) {
    const file = path.join(dir, `${r.slug}.md`);
    assert.ok(existsSync(file), `missing generated recipe "${r.slug}" — run: node scripts/build-agents.mjs`);
    assert.equal(readFileSync(file, 'utf8'), renderRecipe(r), `plugin/recipes/${r.slug}.md is STALE — run: node scripts/build-agents.mjs`);
  }
